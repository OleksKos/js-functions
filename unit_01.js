...
out1.innerHTML = getRandomInt(117, 132);
...
const buttonB3 = document.querySelector('.b-3');
buttonB3.addEventListener('click', t3);
...
function t5() {
    const numberFromInput = +document.querySelector('.i-5').value;
...
// const out2 = document.querySelector('.out-2').innerHTML;

function t6() {
    function odd() {
        out6.textContent = `odd ${OddOrEven} ` + typeof (OddOrEven);
    }
    function even() {
        out6.textContent = `even ${OddOrEven} ` + typeof (OddOrEven);
    }
    min = 10000;
    max = 10099;
    // тут запускаете getRandomInt2
    let OddOrEven = getRandomInt2(min, max);
    out6.innerHTML = OddOrEven;

    if (out6.innerHTML % 2 === 0) {
        odd();
    } else {
        even()
    }
...
function t7() {
    ...
    out7.textContent = z7;
    console.log(typeof (+out7.innerHTML));
