...
function t1(min, max) {
    min = Math.ceil(min);
    max = Math.floor(max);
    out1.innerHTML = Math.round(Math.random() * (max - min) + min);
}
...
function t2(min, max, block) {
    min = Math.ceil(min);
    max = Math.floor(max);
    block.innerHTML = Math.round(Math.random() * (max - min) + min);
}
...
function t3(min = 0, max = 100) {
    min = Math.ceil(min);
    max = Math.floor(max);
    out3.innerHTML = Math.round(Math.random() * (max - min) + min);
}
...
function t7(arr = [], block) {
    ...
    block.innerHTML = Array.isArray(arr) ? arr : false;
}
...
function t9(text = '', block) {
    block.innerHTML = text != '' ? text.trim().toLowerCase() : text = 'yb';
}
...
document.querySelector('.b-10').addEventListener('click', function () {
    t10(33, 22, 44, 11, 55, 66, 11, 66, 66);
})
